...
Última actualización: **2026-09-02** · toda la suite en verde.
...
| Proyectos                          | 6.4       | ✅     | CRUD, aplazar, finalizar, reabrir. **Ya no habilita puestos**: `categorias` y la ruta de clonarlas se quitaron (D-82)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                  |
...
| Subidas directas (`POST /subidas`) | —         | ✅     | El archivo va del navegador a R2 y el servidor sólo autoriza y registra (D-83); el `multipart` de siempre sigue en pie. Falta que el front las use                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                     |
...
> son confirmaciones, constantes o preguntas con fecha —la **#19** conviene
> contestarla antes de la carga real de obras—: se pueden dejar correr sin que se
> detenga nada. Las tachadas ya se resolvieron y se dejan para no repetir la
> discusión.
...
19. **Si el SIROC se tramita tarde, ¿desde dónde se predice cuántos refrendos
    pide el contrato?** Hoy `actualizacionesRequeridas` se cuenta desde
    `fechaInicio` —«cuántos necesita esta obra», útil para presupuestar— mientras
    el vencimiento corre desde `fechaRegistro`, así que un SIROC tramitado siete
    meses tarde muestra una predicción de un calendario y un vencimiento de otro.
    La alternativa es predecir desde el registro: «cuántos quedan por delante»,
    útil para operar, pero entonces no hay número hasta que exista el SIROC — y
    hoy se responde desde el alta precisamente porque no depende de él. Es la
    primera de las dos preguntas de
    `cames-ops/plan/propuestas/2026-09-01-backend-siroc-registro-tardio.md`; la
    segunda la contestó el cliente y es **D-84**. **No bloquea nada**: los dos
    números son correctos por separado y `actualizacionesPendientes` —que es el
    que se pinta— ya se cuenta desde el aviso. **Y desde D-85 casi no tiene
    objeto**: `fechaRegistro` cae a siete días o menos de `fechaInicio`, así que
    los dos calendarios ya no pueden separarse más que eso. Queda abierta sólo
    por si Urbacames relaja esa holgura.
