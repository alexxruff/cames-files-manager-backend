/**
 * Requisitos de entorno: SÓLO la forma, sin efectos.
 *
 * Este módulo se separó de `env.js` para que se pueda LEER lo que el backend
 * necesita sin dispararlo: no llama a dotenv, no toca `process.env`, no valida
 * nada y no termina el proceso. Quien quiera la configuración ya validada usa
 * `src/config/env.js`; quien sólo quiera saber qué variables hacen falta —otro
 * repo, un script de despliegue, `scripts/printEnvRequirements.js`— usa ésta.
 *
 * Si agregas una variable, va aquí y en `.env.example`.
 */
const { z } = require('zod')
...
module.exports = { schema, booleanFromString }
